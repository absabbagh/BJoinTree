...
var
  yyinputText: string;
  yyoutputText: array of string;    (* input and output file *)
  yyline: string;      (* current input line *)
  yylineno{, yycolno}: integer;     (* current input position *)
  yycolno: Integer = 0;
  yytext: string;      (* matched text (should be considered r/o) *)
  yyLexerrmsgs: array of string; (* error messages *)
...
begin
  setLength(yyLexerrmsgs,length(yyLexerrmsgs) + 1);
  yyLexerrmsgs[High(yyLexerrmsgs)] := 'LexLib: ' + msg;
  halt(1);
end(*fatal*);
...
begin
  ...
  { ignore }
    ...
    setLength(yyoutputText,length(yyoutputText)+1)
  else
    yyoutputText[length(yyoutputText)] := yyoutputText[length(yyoutputText)] + c
end(*put_char*);
