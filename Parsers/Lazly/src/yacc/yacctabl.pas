...
{$ELSE}
  ...
  max_types  = 300;  (* number of type tags                         *)
  max_prec   = 50;   (* maximum precedence level                    *)
{$IFDEF Windows}
  max_states         =  1800;  (* number of LR(0) states                      *)
...
{$ELSE}
  max_rule_len  = 64;  (* maximum length of rules                     *)
  max_set_items = 396;  (* maximum number of items in an item set      *)
{$ENDIF}
...
function get_key(symbol: AnsiString): integer;
...
function pname(sym: integer): AnsiString;
...
function search_type(symbol: AnsiString): boolean;
...
{$ifndef fpc}{$F+}{$endif}
function lookup(k: integer): AnsiString;
{$ifndef fpc}{$F-}{$endif}
...
{$ifndef fpc}{$F+}{$endif}
procedure entry(k: integer; symbol: AnsiString);
{$ifndef fpc}{$F-}{$endif}
