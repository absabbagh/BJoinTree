...
type
  ...
  private
    ...
    fLastInsertRowId: DataPointerType;
    ...
    function GetLastInsertRowId: DataPointerType;

    function GetDeletePointer: DataPointerType;

    procedure SetDeletePointer(DeletePointer: DataPointerType);

    function getRowDeletePointer(rowId: DataPointerType): DataPointerType;

    procedure SetRowDeletePointer(rowId: DataPointerType; DeletePointer: DataPointerType);
...
//    procedure putValueByColumnName(rowId: DataPointerType; columnName: string, value: variant);
    ...
    property LastInsertRowId: DataPointerType read GetLastInsertRowId;
...
function TTableClass.GetLastInsertRowId: DataPointerType;
begin
  result := FLastInsertRowId
end;

function TTableClass.GetDeletePointer: DataPointerType;
var
  Offset: Int64;
  Origin: Word;
begin
  result := NullDeleteValue;
  Offset := 0;
  Origin := soFromBeginning;
  Seek(Offset,Origin);
  ReadBuffer(result,SizeOf(DataPointerType))
end;

procedure TTableClass.SetDeletePointer(DeletePointer: DataPointerType);
var
  Offset: Int64;
  Origin: Word;
begin
  ...
  Origin := soFromBeginning;
  Seek(Offset,Origin);
  WriteBuffer(DeletePointer,SizeOf(DataPointerType))
end;

function TTableClass.getRowDeletePointer(rowId: DataPointerType): DataPointerType;
var
  index: Integer = 0;
  buffer: bufferType;
begin
  ...
  Move(buffer[Index],result,sizeof(DataPointerType));
end;

procedure TTableClass.SetRowDeletePointer(rowId: DataPointerType; DeletePointer: DataPointerType);
var
  index: Integer = 0;
  buffer: bufferType;
begin
 if (rowId <= 0 ) or (rowId > fsize) then exit;
 loadLRUBuffer(rowId,buffer);
 Move(DeletePointer,buffer[Index],sizeof(DataPointerType));
...
var
  ...
  DeletePointer: DataPointerType = NullDeleteValue;
begin
  ...
  {$ENDIF}
  FrecordLen := SizeOf(DataPointerType);
  ...
  if isOpen then
    begin
      ...
      inherited Create(FTableName + '.tbl', Params);
    end else
    begin
      Params := fmCreate or fmShareDenyNone;
      inherited Create(FTableName + '.tbl', Params);
      SetDeletePointer(DeletePointer);
    end;
  FOffsetHead := SizeOf(int64);
  Offset := 0;
  Origin := soFromEnd;
  Seek(Offset, Origin);
...
begin
  ...
  Inc(Index, Sizeof(DataPointerType));
...
begin
  ...
  // Check DeletePointer if there is any Deleted Record
  FLastInsertRowId := GetDeletePointer;
  if FLastInsertRowId = NullDeleteValue then
    begin
      fsize := fsize + 1;
      FLastInsertRowId := fSize
    end else
    begin
      setDeletePointer(getRowDeletePointer(FLastInsertRowId));
    end;
  insertBuffer(FLastInsertRowId, row);
end;

function TTableClass.returnRow(rowId: DataPointerType;
  out row: array of variant): boolean;
var
  FDeletePointer: DataPointerType = NullDeleteValue;
  buffer: bufferType;
  index: Integer = 0;
  i, j: integer;
  stType: string;
...
begin
  ...
  if (rowId <= 0 ) or (rowId > fsize) then exit;
  loadLRUBuffer(rowId,buffer);
  Move(buffer[Index],FdeletePointer,sizeof(DataPointerType));
  index := Sizeof(DataPointerType);
  result := true;
  if not ExistRow(rowId) then Exit;
  index := Sizeof(DataPointerType);
    ...
    begin
      ...
      if (stType = 'SMALLINT') then
        begin
          ...
          index := index + Sizeof(smallint);
          if row[I] <> null then row[I] := smallintresult;
        end;
      if (stType = 'INTEGER') then
        begin
          Move(buffer[index],intresult,sizeof(integer));
          index := index + Sizeof(integer);
          if row[I] <> null then row[I] := intresult;
        end;
      if (stType = 'INT64') then
        begin
          Move(buffer[index],int64result,sizeof(int64));
          index := index + Sizeof(int64);
          if row[I] <> null then row[I] := int64result;
        end;
    end;
end;
...
begin
  result := (getRowDeletePointer(rowId) = NullValue)
end;
...
begin
  setRowDeletePointer(rowId,GetDeletepointer);
  setDeletePointer(rowId);
end;
