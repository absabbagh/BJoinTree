...
begin
  ...
  FileExtension := ExtractFileExt(yfilename);
  FilePath := ExtractFileDir(yfilename) + DirectorySeparator;
  FileNameWithoutExtension := StringReplace(yfilename, FileExtension, EmptyStr, []);
  FileNameWithoutPath := StringReplace(FileNameWithoutExtension, FilePath, EmptyStr, []);
  codFileName := FileNameWithoutPath;

  if codfilename = '' then
    codfilename := 'yyparse.cod'
   else
    codfilename := codfilename + '.cod';
  ...
  codfilename := codfilepath + codfilename;
  Assign(yycod, codfilename);
  reset(yycod);
