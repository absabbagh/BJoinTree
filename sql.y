...
%type <Pointer> option_privilege
%type <Pointer> option_privileges
...
sql_transcations_statement : tknSET tknTRANSACTION transaction_name
                               { $$ := opr(207,'SET TRANSACTION',[$3]); }
...
transaction_name : ID
                    { $$ := opr(211,'TRANSACTION NAME',[DBName($1)]); }
                 ;
...
grant_privileges : tknGRANT option_privileges tknON option_object tknTO user_id option_grant_option
               { $$ := opr(231,'GRANT',[$2,opr(240,'DATABASE OBJECT',[$4]),$6]); }
                 ;
...
option_privileges : option_privilege
                      { $$ := $1; }
                  |   option_privileges ',' option_privilege
                      { $$ := opr(0,'REPEAT',[$1,$3]); }
                  ;

option_privilege : tknSELECT
                 ...
                 | tknINDEX
                     { $$ := opr(233,'PRIVILEGE',[stOption('INDEX')]); }
                 | tknCREATE
                     { $$ := opr(233,'PRIVILEGE',[stOption('CREATE')]); }
                 | tknALTER
...
revoke_privileges : tknREVOKE option_privileges tknON ID tknFROM user_id
               { $$ := opr(232,'REVOKE',[$2,DBName($4),$6]); }
                 ;
...
cursor_declaration : tknDECLARE cursor_name tknCURSOR tknFOR select_command
                    { $$ := opr(220,'END CURSOR DECLARATION',[opr(216,'START CURSOR DECLARATION',[$2]),$5]); }
                   ;
...
select_sys_command : tknSELECT tknSYSTEM tknDATABASES tknFOR user_id
                    { $$ := opr(157,'SHOW ALL DATABASES',[$5]); }
                    | tknSHOW tknDATABASES
                    { $$ := opr(157,'SHOW ALL DATABASES',[]); }
                    | tknSHOW tknDATABASES tknFOR user_id
                    { $$ := opr(157,'SHOW ALL DATABASES',[$4]); }
                    | tknSELECT tknSYSTEM database_name tknFROM tknDATABASES
                    { $$ := opr(159,'SWITCH DATABASE',[$3]); }
                    | tknUSE database_name
                    { $$ := opr(159,'SWITCH DATABASE',[$2]); }
                    | tknSHOW tknTABLES
                    { $$ := opr(160,'SHOW ALL TABLES',[]); }
                    | tknSELECT tknSYSTEM tknTABLES tknFROM database_name
                    { $$ := opr(160,'SHOW ALL TABLES',[$5]); }
                    | tknSHOW tknCOLUMNS tknFROM table_name
                    { $$ := opr(161,'SHOW ALL COLUMNS',[$4]); }
                    | tknSELECT tknSYSTEM tknCOLUMNS tknFROM table_name
                    { $$ := opr(161,'SHOW ALL COLUMNS',[$5]); }
                    | tknSELECT tknSYSTEM tknJOININDEXES tknFROM database_name
                    { $$ := opr(162,'SHOW ALL JOIN INDEXES',[$5]); }
                    | tknSELECT tknSYSTEM tknINDEXES tknFROM table_name
                    { $$ := opr(163,'SHOW ALL INDEXES',[$5]); }
                    | tknSELECT tknSYSTEM tknCONSTRAINTS tknFROM table_name
                    { $$ := opr(164,'SHOW ALL CONSTRAINTS',[$5]); }
